...
% 14.05.2014
  ...
  % We need to convert the previously selected types and compression
  % to their corresponding index in their respective drop-down lists.
  contents = get(handles.channel_type,'String');
  compressions = get(handles.compress,'String');

  % Loop over the different channels
  for i=1:nchannels
    % Try to find the index of the prefered type, otherwise taje the first one
    tmp_indx = find(ismember(contents, channels(i).type), 1);
    if (isempty(tmp_indx))
      channels(i).type = 1;
    else
      channels(i).type = tmp_indx;
    end

    % Same for the compressions
    tmp_indx = find(ismember(compressions, channels(i).compression), 1);
    if (isempty(tmp_indx))
      channels(i).compression = 1;
    else
      channels(i).compression = tmp_indx;
    end
  end
